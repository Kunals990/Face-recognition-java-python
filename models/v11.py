...
while True:
    ...
    # Initialize a list to keep track of recognized names in the current frame
    recognized_names = []
                        ...
                        if matches[matchIndex]:
                            ...
                            if name in marked_attendance:
                                cv2.putText(img, 'Attendance Marked', (x1 + 6, y1 - 6), cv2.FONT_HERSHEY_COMPLEX, 0.5,
                                            (255, 0, 0), 1)  # Change color to blue
                else:
                    color = (0, 0, 255)

                cvzone.cornerRect(img, (x1, y1, w, h), colorC=color, colorR=color)
                cvzone.putTextRect(img, f'{yolo_classNames[cls].upper()} {int(conf * 100)}%', (max(0, x1), max(35, y1)),
    ...
    # Mark attendance for each recognized face in the current frame
